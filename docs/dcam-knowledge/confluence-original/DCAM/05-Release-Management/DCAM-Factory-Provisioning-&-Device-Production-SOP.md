...
Phase 2 / Secure Platform MVP Build 0.2 only: Factory Worker login, scan QR trong Workspace, nhập/chọn owner/manufacture date và submit create/restore request under DEC-P2-WEB-01. This is not a factory acceptance or shipment decision.
...
[Decision Brief – DCAM Phase 2 Web Portal Scope Precedence](/wiki/spaces/DVID/pages/54296681/Decision+Brief+DCAM+Phase+2+Web+Portal+Scope+Precedence) applies this business-provisioning step only to Phase 2 / `Secure Platform MVP Build 0.2`. It does not set Device Owner, inject Android serial, mark `READY_TO_SHIP`, complete factory acceptance or approve shipment.
