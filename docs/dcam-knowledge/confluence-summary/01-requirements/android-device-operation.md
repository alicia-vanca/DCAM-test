...
Source status: current Confluence page version 13, last updated 2026-08-26. This is a local implementation-oriented digest; the [Confluence requirement](https://ducviet.atlassian.net/wiki/spaces/DVID/pages/48496661) remains authoritative.
...
| No external EMM baseline | Current baseline must not depend on external EMM, Android Management API, or Managed Google Play policy-driven update. Headwind Client is Application mode only. |
...
| Operator session | Later profiles may require an active operator session. Build 0.1 uses its approved fixed technical operator exception and does not add login UI. Emergency recording may use `EMERGENCY_OVERRIDE_ADMIN` when policy allows. |
...
| GMS-free runtime | Production Android is GMS-free: no Google Play services, Play Store, Google account, FCM, Analytics, or Play Integrity dependency/flow. |
| Self Update | BFF-authorized immutable Cloudflare R2/CDN artifacts are the production primary path; Play Store/Managed Google Play/Google-account update is rejected. |
