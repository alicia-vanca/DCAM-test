...
final class EglFanOutFastProbeTest {
    ...
    @Test void fullCartesianKeepsOnlySupportedRawTuples(){
        CaptureModeTuple pass=new CaptureModeTuple(V30,I1);LoggerStub log=new LoggerStub();
        EglFanOutFastProbe.Result result=run(List.of(V30,V60),List.of(I1,I2),(tuple,a,c)->
                tuple.equals(pass)?EglFanOutFastProbe.Decision.supported("ok"):EglFanOutFastProbe.Decision.rejected("no"),log);
        ...
        assertTrue(result.evidence().candidateEvidence().isEmpty());assertTrue(log.lines.isEmpty());
    ...
    @Test void destructiveFalseGetsOneConfirmation(){
        ...
        EglFanOutFastProbe.Result result=run(List.of(V30,V60),List.of(I1,I2),(tuple,a,c)->{counts.merge(tuple,1,Integer::sum);return tuple.equals(pass)?EglFanOutFastProbe.Decision.supported("ok"):EglFanOutFastProbe.Decision.rejected("no");},new LoggerStub());
    ...
    @Test void globalFailureLeavesEarlierFactsUnknown(){
        ...
        EglFanOutFastProbe.Result result=run(List.of(V30),List.of(I1,I2),(tuple,a,c)->tuple.equals(pass)?EglFanOutFastProbe.Decision.supported("ok"):EglFanOutFastProbe.Decision.unbuilt(EglFanOutFastProbe.AttemptResult.INCOMPLETE_GLOBAL,"egl_lost"),new LoggerStub());
    ...
    @Test void pipelineBFailureDoesNotMutateA(){
        ...
        EglFanOutFastProbe.Result b=run(List.of(V30),List.of(I1),(t,x,c)->EglFanOutFastProbe.Decision.rejected("b_fail"),new LoggerStub());
    ...
    static EglFanOutFastProbe.Result run(List<VideoMode> v,List<ImageMode> i,EglFanOutFastProbe.TupleQuery q,LoggerStub l){return EglFanOutFastProbe.runMatrix(CAMERA,v,i,q,l,System.nanoTime());}
    static final class LoggerStub implements Logger{final List<String> lines=new ArrayList<>();public void debug(String m){lines.add(m);}public void info(String m){lines.add(m);}public void info(String m,Throwable e){lines.add(m);}public void warn(String m,Throwable e){lines.add(m);}public void error(String m,Throwable e){lines.add(m);}}
}