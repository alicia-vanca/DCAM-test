...
public final class EglFanOutFastProbeTest {
    @Test public void reportsPrivateJpegCameraOutputsAndEglDownstreams() throws Exception {
        ...
        EglFanOutFastProbe.Result result = new EglFanOutFastProbe(
        ...
        assertEquals(EglFanOutFastProbe.PIPELINE_ID,
                result.evidence().verificationPipelineId());
        assertTrue(result.evidence().candidateEvidence().isEmpty());
        assertTrue("EGL cleanup incomplete", result.cleanupComplete());
        if (result.completion()
                == EglFanOutFastProbe.Completion.PIPELINE_UNAVAILABLE) {
        ...
        assertEquals("Device probe incomplete: " + result.detail(),
                EglFanOutFastProbe.Completion.COMPLETE, result.completion());
        ...
        assertTrue(result.attempts().stream().allMatch(attempt ->
                attempt.cameraOutputCount() == 2
                        && attempt.downstreamSurfaceCount() == 2
                        && attempt.cameraSurfaceClasses().equals("SurfaceTexture|ImageReader")
                        && attempt.downstreamSurfaceClasses().equals(
                                "SurfaceTexture+MediaCodec")));
    }
    ...
    private static void print(String message) {
        System.out.println("EGL_FANOUT_DIAGNOSTIC " + message);
    }
